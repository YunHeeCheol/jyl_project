// CommNetwork.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CCommNetwork, CCmdTarget)

	CCommNetwork::CCommNetwork()
{
...
CCommNetwork::CCommNetwork(CString port,CString baudrate,CString parity,CString databit,CString stopbit, int nFlag)
{
	m_sComPort = port;
...
BEGIN_MESSAGE_MAP(CCommNetwork, CCmdTarget)
	//{{AFX_MSG_MAP(CCommNetwork)
	// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CCommNetwork message handlers



BOOL CCommNetwork::Create(HWND hWnd)
{
	m_hWnd = hWnd;
...
void CCommNetwork::ResetSerial()
{
	...
	//설정된 DCB를 통신포트의 제어 파라메터로 설정
	SetCommState(m_hComDev, &dcb);
	//버퍼에 데이터가 들어왓을때 이벤트 발생시킴
	SetCommMask(m_hComDev,EV_RXCHAR);
}

BOOL CCommNetwork::Send(LPCTSTR outbuf, int len)
{
	BOOL	bRet=TRUE;
	DWORD       ErrorFlags;
	COMSTAT     ComStat;
...
int CCommNetwork::Receive(LPSTR inbuf, int len)
{
...
UINT CommThread(LPVOID lpData)
{
	...
	CCommNetwork* Comm = (CCommNetwork*)lpData;
	...
	while (Comm->m_bIsOpenned) {
		...
		if ((EvtMask & EV_RXCHAR) && ComStat.cbInQue) {
			SendMessage(Comm->m_hWnd,WM_MYRECEIVE2,insize,temp);
			...
			memcpy(Comm->m_sInBuf,buf,insize);
...
int CCommNetwork::FindCommPort(CString PortArr[])
{	
	int nCntPort = 0;
	...
	for ( int i=0; i<20; i++ )
	{
		temPortstr.Format("\\\\.\\COM%d",i);

		// 시리얼 통신 포트 열기
		hTempComm = CreateFile( temPortstr, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL );

		// 열기 실패_포트 없음
		if ( hTempComm == INVALID_HANDLE_VALUE )
		{
			continue;
		}
		// 열기 성공_포트 존재
		...
		{
			PortArr[nCntPort]=temPortstr;
			nCntPort++;
			CloseHandle(hTempComm);
		}
	}
	return nCntPort;
}
